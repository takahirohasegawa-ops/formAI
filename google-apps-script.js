...
// === 設定 ===
const API_ENDPOINT = 'https://web-production-98b1.up.railway.app/api/submit';
...
function submitRows(sheet, startRow, numRows) {
  for (let i = 0; i < numRows; i++) {
    ...
    try {
      ...
      sheet.getRange(row, 3).setValue('⚠️ エラー');
      sheet.getRange(row, 3).setBackground('#f8d7da');
      sheet.getRange(row, 4).setValue('タイムアウト: ' + error.toString());
    }
...
function callFormAPI(url, message) {
  ...
  const options = {
    ...
    timeout: 300  // タイムアウトを5分(300秒)に設定
  };
